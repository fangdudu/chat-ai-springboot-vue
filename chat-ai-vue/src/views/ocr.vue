          ...
          <!-- 图片输入和按钮区域 -->
           <div class="flex items-center gap-1 min-w-0">
            ...
            <button @click="$refs.fileInput.click()"
              class="flex-1 p-2 text-gray-500 hover:text-gray-700 hover:bg-gray-100 rounded-full transition-colors flex items-center justify-center h-10"
              title="上传图片">
              ...
              <span class="text-sm">请上传图片</span>
            </button>

            <!--输入按钮-->
            <button v-if="!isChatting"
              class="bg-blue-500 text-white rounded-full p-1.5 ml-2 hover:bg-blue-600 focus:outline-none flex items-center justify-center w-8 h-8"
              @click="sendMessage">
            ...
            <button v-if="isChatting"
              class="bg-red-400 text-white rounded-full p-1.5 ml-2 hover:bg-red-500 focus:outline-none focus:bg-red-500 flex items-center justify-center w-8 h-8"
              @click="suspendChat">
              <!-- 暂停图标 -->
              <svg class="w-5 h-5" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
